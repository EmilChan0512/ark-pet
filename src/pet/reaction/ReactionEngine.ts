...
export class ReactionEngine {
  ...
  private snapshot: ReactionEngineSnapshot = {
    ...
    activeState: 'idle', blockedReason: null, generation: 0,
  }
  ...
  handle(event: ContextEvent): string | null {
    ...
    try {
      void Promise.resolve(this.execution.execute(plan, token)).then(
        () => { if (token.isValid()) { this.active = null; this.patch({ activeReactionId: null, activeState: 'idle' }) } },
        (error) => { if (token.isValid()) { this.report(selected.id, error); this.active = null; this.patch({ activeReactionId: null, activeState: 'idle' }) } },
      )
    } catch (error) {
